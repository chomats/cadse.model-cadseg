...
public final class CadseDefinitionCIF implements IContentItemFactory {
	 ...
	 */
	public static class WorkspaceModelContentManager extends EclipsePluginContentManger implements IPDEContributor {
		 ...
		 *            the item
		 */
		private WorkspaceModelContentManager(final Item item) {
			super(item, new VariableImpl() {

				public String compute(ContextVariable context, Item item) {
					return CadseDefinitionManager.RESOURCE_SUFFIX
							+ context.getAttribute(item, CadseRootCST.ITEM_TYPE_at_NAME_);
				}
			}, true);
		}
		...
		public void computeManifest(OsgiManifest omf) {
			...
			for (Item item : dep) {
				omf.putArray(OsgiManifest.REQUIRE_BUNDLE, true, false, CadseDefinitionManager.getUniqueName(item));
			}
	...
	public ContentItem createContentItem(Item item) throws CadseException {
		return new WorkspaceModelContentManager(item);
	}
}