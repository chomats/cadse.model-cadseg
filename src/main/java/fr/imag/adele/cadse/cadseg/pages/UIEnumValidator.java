...
class UIEnumValidator extends AbstractUIRunningValidator {
/**
...
public static List<String> getEnumTypeValues(IType type) {
	...
	public void notifieValueChanged(UIField field, Object value) {
		if (field.getAttributeDefinition() == CadseGCST.ENUM_TYPE_at_JAVA_CLASS_) {
			...
			if (javaClass != null) {
				...
				_uiPlatform.setVisualValue(CadseGCST.ENUM_TYPE_at_VALUES_, values, false);
				EnumTypeManager.setMustBeGeneratedAttribute(_uiPlatform.getItem(), false);
				_uiPlatform.setEnabled(CadseGCST.ENUM_TYPE_at_VALUES_, false);
			} else {
				_uiPlatform.setEnabled(CadseGCST.ENUM_TYPE_at_VALUES_, true);
				EnumTypeManager.setMustBeGeneratedAttribute(_uiPlatform.getItem(), true);
			}
		}
	}
	...
	public void initAfterUI() {
		List<String> values = EnumTypeManager.getEnumTypeValues(_uiPlatform.getItem());
		_uiPlatform.setVisualValue(CadseGCST.ENUM_TYPE_at_VALUES_, values, false);
		boolean g = EnumTypeManager.isMustBeGeneratedAttribute(_uiPlatform.getItem());
	...
	public boolean validValue(UIField field, Object value) {
		if (field.getAttributeDefinition() == CadseGCST.ENUM_TYPE_at_JAVA_CLASS_) {
			...
			if (!booleanValue) { // not generated
				IType javaClass = value instanceof IType ? (IType) value : null;
				if (javaClass == null) {
					_uiPlatform.setMessageError("You must select a java enum type");
					return true;
				}
			}
		}
