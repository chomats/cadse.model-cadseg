...
public class ImportCadseUtil {
	...
	static public String readCadse(File f) throws IOException, JAXBException {
		...
		ZipEntry entry = jis
				.getEntry(ExportCadsePagesAction.MELUSINE_DIR_CADSENAME);
		if (entry == null) {
			entry = jis.getEntry("/"
					+ ExportCadsePagesAction.MELUSINE_DIR_CADSENAME);
			if (entry == null) {
				throw new IOException("Cannot found "
						+ ExportCadsePagesAction.MELUSINE_DIR_CADSENAME);
			}
		}
	 ...
	 *             the JAXB exception
	 */
	static public CompactUUID readCadseUUID(File f) throws IOException,
			JAXBException {
		JarFile jis = new JarFile(f);
		ZipEntry entry = jis
				.getEntry(ExportCadsePagesAction.MELUSINE_DIR_CADSENAME_ID);
		if (entry == null) {
			entry = jis.getEntry("/"
					+ ExportCadsePagesAction.MELUSINE_DIR_CADSENAME_ID);
			if (entry == null) {
				throw new IOException("Cannot found "
						+ ExportCadsePagesAction.MELUSINE_DIR_CADSENAME_ID);
			}
	...
	static private void migrate(LogicalWorkspaceTransaction transaction)
		...
		for (ItemDelta itemDelta : operations) {
			...
			SetAttributeOperation uuid_att = itemDelta
					.getSetAttributeOperation("UUID_ATTRIBUTE");
			if (uuid_att != null) {
				if (itemDelta.getType() == null) {
					System.out.println("Import erroor type is null");
				} else if (itemDelta.getType() == CadseGCST.CADSE_DEFINITION) {
					itemDelta.setAttribute(
							CadseGCST.CADSE_DEFINITION_at_ID_RUNTIME_, uuid_att
									.getCurrentValue());
				} else if (itemDelta.isInstanceOf(CadseGCST.PAGE)) {
					itemDelta.setAttribute(CadseGCST.PAGE_at_ID_RUNTIME_,
							uuid_att.getCurrentValue());
				} else if (itemDelta.isInstanceOf(CadseGCST.ATTRIBUTE)) {
					itemDelta.setAttribute(CadseGCST.ATTRIBUTE_at_ID_RUNTIME_,
							uuid_att.getCurrentValue());
				} else if (itemDelta.isInstanceOf(CadseGCST.ABSTRACT_ITEM_TYPE)) {
					itemDelta.setAttribute(
							CadseGCST.ABSTRACT_ITEM_TYPE_at_ID_RUNTIME_,
							uuid_att.getCurrentValue());
				} else {
					System.out.println("Cannot set UUID_ATTRIBUTE for type "
							+ itemDelta.getType().getName());
				}
				// remove old valeur
			...
			if (itemDelta.getType() == CadseGCST.LINK) {
				...
				LinkDelta l = itemDelta
						.getOutgoingLink(CadseGCST.LINK_lt_INVERSE_LINK);
				if (l != null
						&& l.getDestination().getName().startsWith(
								"#invert_part")) {
					l.delete();
				}
			}
			...
			if (committed_date_value != null) {
				if (committed_date_value.getCurrentValue() instanceof Date) {
					...
					itemDelta.setAttribute(CadseGCST.ITEM_at_COMMITTED_DATE_, d
							.getTime());
				}
			}
		}
		...
		for (ItemDelta itemDelta : operations) {
			...
			for (LinkDelta l : itemDelta.getOutgoingLinkOperations()) {
				if (l.getLinkTypeName().startsWith("#parent:")
					...
					if (itemDelta.getOutgoingLink(CadseGCST.ITEM_lt_PARENT) == null) {
						itemDelta.createLink(CadseGCST.ITEM_lt_PARENT, l
								.getDestination());
					}
					l.delete();
				}
				...
				if (l.getLinkType() != null && l.getLinkType().isPart()
						&& l.getDestination().getPartParent() == null) {
					l.getDestination()
							.setParent(l.getSource(), l.getLinkType());
				}
			}
			for (LinkDelta l : itemDelta
				...
				if (l.getDestination().getType() == null) {
					...
					if (att != null) {
						LinkDelta latt = itemDelta.getOutgoingLink(
								CadseGCST.ITEM_lt_MODIFIED_ATTRIBUTES, att
										.getId());
						if (latt != null) {
							l.delete();
						} else
		...
		for (ItemDelta itemDelta : operations) {
			if (!itemDelta.isLoaded())
				continue;
			if (itemDelta.getPartParent() == null
					&& itemDelta.getType() != null
					&& itemDelta.getType().isPartType()) {
				System.out.println("Error cannot found parent for "
						+ itemDelta.getQualifiedName());
			}
		}
	}
	...
	static private void checkAction(LogicalWorkspaceTransaction transaction) {
		...
		for (ItemDelta itemDelta : operations) {
			...
			if (parent == null && itemDelta.getPartParent() != null) {
				System.err.println("Parent not setted " + itemDelta + " -> "
						+ itemDelta.getPartParent());
			} else {
				if (parent != null && itemDelta.getPartParent() != null) {
					if (!parent.getId().equals(
							itemDelta.getPartParent().getId())) {
						System.err.println("Parent not same " + itemDelta
								+ " -> " + itemDelta.getPartParent() + "<>"
								+ parent);
					}
				}
	...
	/**
	 * 
	 * @param cadse the name of the cadse : can be null, it read from
	 * @param input
	 * @return 
	 * @throws IOException
	 * @throws CadseException
	...
	static public Item importCadse(String cadse, InputStream input) throws IOException, CadseException {
		...
		try {
			...
			ProjectAssociation pa = new ProjectAssociation(uuid,
					cadse);
			projectAssociationSet.add(pa);
			LogicalWorkspaceTransaction transaction = CadseCore
	...
	/**
	 * 
	 * @param dir can be null : see {@link File#createTempFile(String, String, File)}.
